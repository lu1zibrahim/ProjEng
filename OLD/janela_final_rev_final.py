...
def make_window1():
    ...
    layout = [[sg.Column([
            [sg.Image('UI/images/unifesp.png')],
              [sg.Button("", image_filename='UI/Icons/acessar_camera.png', button_color=(sg.theme_background_color(),sg.theme_background_color()),font=('Bookman Old Style', 1),border_width=0,key="Janela_verificar"),
              ...
              sg.Button('Monitoramento', size=(10, 1), font='Helvetica 14')],
...
def make_window2():
    sg.theme('Black')
    layout = [[sg.Column([
        [sg.Text('Câmera', size=(40, 1), justification='center', font='Helvetica 20')], # Texto para a Camera
...
def make_window5():
    sg.theme('Black')
    headings = ['Nome', 'Entrada','Maximo','Situacao']
    layout = [[sg.Table(values = registros_entrada, headings = headings,
              ...
              [sg.Button("", image_filename='UI/Icons/PaginaInicial.png', button_color=(sg.theme_background_color(),sg.theme_background_color()),font=('Bookman Old Style', 1),border_width=0,key="Home")],
              [sg.Button("", image_filename='UI/Icons/acessar_camera.png', button_color=(sg.theme_background_color(),sg.theme_background_color()),font=('Bookman Old Style', 1),border_width=0,key="Atualizar")]]
...
while True:
                                ...
                                if comparacao == [True]:
                                    ...
                                    try: 
                                        ...
                                        if len(registros_entrada) == 0:
                                            ...
                                            """
                                            agora = datetime.now()
                                            maximo = agora + timedelta(seconds=limit_work)
                                            situacao = 'Normal'
                                            registros_entrada.append([nome,agora,maximo,situacao])
    ...
    if window == window5:
        ...
        tempo_maximo = timedelta(seconds=10)
        reg_entrada = pd.DataFrame(registros_entrada, columns = ['Nome','Entrada','Maximo','Situacao'])
        print(reg_entrada)
        ...
        while True:
            ...
            if event == "Atualizar" and len(registros_entrada) != 0:
                ...
                sg.popup('A sua lista está vazia')
                monitorando = False
            while monitorando:
                ...
                print(comparacao)
                print(passou_horario)
                if True in passou_horario:
                    reg_entrada.loc[reg_entrada['Compare'] == True, 'Situacao'] = 'Excedente' 
                    new_reg = reg_entrada.loc[(reg_entrada['Compare'] == True), ['Nome','Entrada','Maximo','Situacao']]
                    registros_entrada = new_reg.values.tolist()
                    window5['-TABLE-'].update(values = registros_entrada)
            ...
            # for i in registros_entrada:
            #     tem_que_sair = i[1] + tempo_maximo
            #     if datetime.now() > tem_que_sair:
            #         sg.popup(f'Funcionário {i[0]}, passou do tempo maximo')
            #         sleep(10)
